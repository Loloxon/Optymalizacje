        ...
        for (i = k + 1; i < SIZE; i++) {
            ...
            for (j = k + 1; j < SIZE;) {
                if (j < max(SIZE - BLKSIZE, 0)) {
                    ...
                    tmp0 = _mm_loadu_pd(A + IDX (i, j+2, SIZE));
                    tmp1 = _mm_loadu_pd(A + IDX (k, j+2, SIZE));
                    tmp1 = _mm_mul_pd(tmp1, mm_multiplier);
                    tmp0 = _mm_sub_pd(tmp0, tmp1);
                    _mm_storeu_pd(A + IDX (i, j+2, SIZE), tmp0);
                    
                    tmp0 = _mm_loadu_pd(A + IDX (i, j+4, SIZE));
                    tmp1 = _mm_loadu_pd(A + IDX (k, j+4, SIZE));
                    tmp1 = _mm_mul_pd(tmp1, mm_multiplier);
                    tmp0 = _mm_sub_pd(tmp0, tmp1);
                    _mm_storeu_pd(A + IDX (i, j+4, SIZE), tmp0);
                    
                    tmp0 = _mm_loadu_pd(A + IDX (i, j+6, SIZE));
                    tmp1 = _mm_loadu_pd(A + IDX (k, j+6, SIZE));
                    tmp1 = _mm_mul_pd(tmp1, mm_multiplier);
                    tmp0 = _mm_sub_pd(tmp0, tmp1);
                    _mm_storeu_pd(A + IDX (i, j+6, SIZE), tmp0);
